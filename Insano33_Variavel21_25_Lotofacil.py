...
def calculate_frequencies(df, last_n=7):
    ...
    # Identificar números que saíram menos nos últimos 7 sorteios
    last_7 = df.tail(last_n)
    ...
    hot = [num for num, _ in sorted_numbers[:12]]  # Pegar os 12 mais quentes
    stable = [num for num, _ in sorted_numbers[12:22]]
    cold = [num for num, _ in sorted_numbers[22:]]
...
def is_balanced(game):
    ...
    # Permitir variação nas faixas
    range_check = (3 <= ranges[0] <= 5) and (2 <= ranges[1] <= 4) and (2 <= ranges[2] <= 4) and (2 <= ranges[3] <= 4) and (2 <= ranges[4] <= 4)
...
def generate_game(hot, stable, cold, hot_percent, stable_percent, cold_percent, existing_games):
    ...
    while True:
        game = []
        # Escolher aleatoriamente 11 quentes entre os 12 mais quentes
        selected_hot = random.sample(hot, hot_count)
        game.extend(selected_hot)
...
def generate_games_for_concurso(file_path, target_concurso):
    # Janela de 2000 sorteios anteriores
    start_concurso = max(1, target_concurso - 2000)
    ...
    games = []
    existing_games = []
    # 12 jogos principais (Insano33_Variavel21_25)
    for _ in range(12):
        game = generate_game(hot, stable, cold, 0.77, 0.13, 0.10, existing_games)
        games.append(('Insano33_Variavel21_25', game))
        existing_games.append(game)
    
    # 3 jogos Turbo (Insano33_Variavel21_25-Turbo)
    for _ in range(3):
        game = generate_game(hot, stable, cold, 0.77, 0.13, 0.10, existing_games)
        games.append(('Insano33_Variavel21_25-Turbo', game))
        existing_games.append(game)
    
    # 1 jogo Azarão (Insano33_Variavel21_25-Azarão)
    for _ in range(1):
        game = generate_game(hot, stable, cold, 0.70, 0.20, 0.10, existing_games)
        games.append(('Insano33_Variavel21_25-Azarão', game))
